...
{
    // [ИЗМЕНЕНИЕ]: Класс называется Form4
    public partial class Form4 : Form
    {
        // --- Константы и переменные ---
        // [ИЗМЕНЕНИЕ]: Размер сетки 16x16 (Высокая сложность)
        private const int GridSize = 18;
        private const int ButtonSize = 30; // Уменьшим размер кнопки для 16x16, чтобы поместилась на экране
        ...
        private readonly List<string> targetWords = new List<string> {
            "ЛЕС", "СОН", "РЕКА",
            "ПИЛА", "СОЛЬ", "СИСТЕМА", "ОБЛАКО",
            "КНИГА", "МОРЕ", "УСПЕХ", "ВЕТЕР",
            "ВРЕМЯ", "ЦИФРА", "КОД", "СЕТЬ"
        };
        ...
        public Form4()
        {
            ...
            InitializeComponent();
            this.Text = "Филворд: Высокая сложность (16x16)";
            this.Load += Form4_Load; // Изменено имя метода загрузки
        ...
        {
            // Настройка GRID и ListBox и центрирование
        ...
        private void InitializeGameGrid()
        {
            ...
            for (int i = 0; i < GridSize; i++)
            {
                for (int j = 0; j < GridSize; j++)
                {
                    ...
                    btn.Font = new Font(btn.Font.FontFamily, 10, FontStyle.Bold); // Уменьшен шрифт
        ...
        /// <summary>
        /// Фиксированная сетка 16x16 для высокой сложности.
        /// </summary>
        private void LoadFixedGrid()
        {
            // Фиксированные данные сетки 16x16 (должно быть 16 строк по 16 символов)
